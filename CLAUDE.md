...
- **Extraction invariant:** `Extraction → Validation → Human Approval`. LLM-drafted card data lands on a `data/<card-id>` branch as a reviewable diff — never directly on `main`. Every drafted rule carries a citation (file, page, section) and a confidence level; LOW confidence must be flagged for human review. The validator cannot catch hallucinated business rules; the human review step exists for that. **Commit-before-claim rule:** any session touching `data/cards.json` or `research/` must commit its changes to a `data/*` branch BEFORE writing a changelog row or ticking a roadmap checkbox — docs claiming uncommitted work is how the project lost three tasks of data curation (2026-07-11 audit, `docs/launch-directive.md` I-1).
